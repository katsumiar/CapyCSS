...
* ノードのリスト引数が開閉できるようになりました。
* Enum型のSwitchケースノードが作成できるようになりました。
* 変数周りの危険な不具合をいくつか改修しました。
* 接続されたノードの帰り値がより適切にUIに反映されるようになりました。
* インターフェイス引数に List 型のノードを接続して参照渡しできるようになりました。
* 参照渡しで書き換わった変数の中身がスクリプト実行後にUIに反映されるようになりました。
...
* List型からArray型（この逆も）へのキャストができます。
