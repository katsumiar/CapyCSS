...
* UIからの型作成時にジェネリックパラメータに与える型のフィルターを（少なくとも前よりも）正しく動くようにしました。
* エントリーポイント名の重複を禁止しました。
* エントリーポイント名指定での呼び出しサンプルを追加しました。
* ファイルに関連付けして実行した時にCtrl+Oでファイルを開こうとすると落ちる問題に対応しました。
* 実行オプションが正しく動かなくなっていた問題に対応しました。
