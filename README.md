...
* ツール終了時に保存していないプロジェクトの保存確認をするようにしました。
* 編集中のスクリプトを再度読み込もうとしたとき、破棄して再読み込みできるようにしました。
* NuGetよりインポートするときにnet6.0及びnet5.0も対象に含めました。
...
### 0.5.0.2 で対応した不具合
* 引数名変更時にリンクが切れる問題に対応しました。
* プロジェクトをクリアしたときにプロジェクト名が初期状態に戻らない問題に対応しました。
* インポートするものが無いのにインポート操作をしたときにカーソル形状が戻らない問題に対応しました。
* テキスト領域をコピー＆ペーストすると領域内のノードに正しくアクセスできなくなる問題に対応しました。
